...
function Dashboard() {
  ...
  const { firms, loading, incomes, outcomes, total } = useSelector(
    (state) => state.firm
  );
  ...
  return (
    <>
      <div
        className={
          "alert alert-secondary sticky-top mb-4 d-flex gap-4 gap-sm-5 justify-content-center"
        }
      >
        <span className={"h5 footer-text text-success text-center"}>
          Барча киримлар: &nbsp;
          <span className={"d-inline-block mt-2 d-sm-inline mt-sm-0"}>
            {incomes?.toLocaleString("ru-RU")}
          </span>
        </span>
        <span className={"h5 footer-text text-danger text-center"}>
          Барча чикимлар: &nbsp;
          <span className={"d-inline-block mt-2 d-sm-inline mt-sm-0"}>
            {outcomes?.toLocaleString("ru-RU")}
          </span>
        </span>
        <span className={"h5 footer-text text-center"}>
          Жами: &nbsp;
          <span className={"d-inline-block mt-2 d-sm-inline mt-sm-0"}>
